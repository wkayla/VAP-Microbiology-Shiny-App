...
antibiotics_unique_merge <- left_join(med_group,antibiotics,c("X_ANTIMICROBIALMED"="X_ANTIMICROBIALMED"))%>%
    add_count()

antibiotics_unique_merge <- antibiotics_unique_merge%>%
    mutate(duration = as.Date(X_ANTIMICROBIALSTOPDATE,format="%Y-%m-%d")-as.Date(X_ANTIMICROBIALSTARTDATE,format="%Y-%m-%d"),
